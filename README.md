...
cp .env.example .env      # (optional) fill in GEMINI_API_KEY for shared deploys
python main.py
```
...
|---|---|---|---|
| `GEMINI_API_KEY` | `backend/.env` | optional | Server-side fallback. If unset, every user must paste their own key. |
| `ALLOWED_ORIGINS` | backend env | **production** | Comma-separated list of allowed frontend origins (e.g. `https://your-app.vercel.app`). Defaults to localhost — production must override. |
| `ENVIRONMENT` | backend env | optional | `production` disables `/docs` & `/redoc`. Default: `development`. |
| `NEXT_PUBLIC_API_URL` | frontend env | **production** | The backend's public URL. Without it, the production build will fail to call the API. |
